...
import {
  ...
  type UpdateRestaurantFiscal,
  type UpdateRestaurantPaymentMethods,
  type Branch,
  type InsertBranch,
  type UpdateBranch,
  type Table,
...
export interface IStorage {
  ...
  updateRestaurantPaymentMethods(restaurantId: string, data: UpdateRestaurantPaymentMethods): Promise<Restaurant>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateRestaurantFiscal(restaurantId: string, data: UpdateRestaurantFiscal): Promise<Restaurant> {
    const [updated] = await db
      .update(restaurants)
      .set({
        ...data,
  ...
  async updateRestaurantPaymentMethods(restaurantId: string, data: UpdateRestaurantPaymentMethods): Promise<Restaurant> {
    const [updated] = await db
      .update(restaurants)
      .set({ paymentMethods: data.paymentMethods, updatedAt: new Date() })
      .where(eq(restaurants.id, restaurantId))
      .returning();
    return updated;
  }
  ...
  async updateUser(restaurantId: string | null, id: string, data: { email?: string; firstName?: string; lastName?: string; profileImageUrl?: string; role?: 'superadmin' | 'admin' | 'manager' | 'cashier' | 'waiter' | 'kitchen' }): Promise<User> {
  ...
  async getKitchenOrders(restaurantId: string, branchId?: string | null, includeAwaitingConfirmation = true): Promise<Array<Order & { customer: Customer | null; table: Table | null; orderItems: Array<OrderItem & { menuItem: MenuItem; options?: OrderItemOption[] }> }>> {
    ...
    // The POS must see public orders while the operator is confirming payment.
    // Only the kitchen queue should require a paid order.
    const paidCondition = includeAwaitingConfirmation
      ? undefined
