...
**How to apply:** Keep upload URLs rooted at the backend, preserve a legacy read path during migration, configure persistent storage before relying on uploads, and make each SQL migration idempotent while checking actual snake_case schema names, dependency order, and the physical columns created by startup initialization. In bundled ESM production builds, resolve migration files from the project source path as well as beside the compiled entrypoint because `__dirname` points into `dist`.