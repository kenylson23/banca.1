...
The open-cash-shift rule must be enforced at the shared payment-writing boundary as well as in individual routes. It applies to operational table and counter/PDV orders, including orders submitted through the public menu, regardless of whether the operator is a cashier, manager, administrator, or anonymous customer. Public-menu rejection should use customer-facing wording, while internal payment flows retain the exact operational cash-register message. Confirmation endpoints can bypass the normal cashier payment route and otherwise create payment records while the cash register is closed.
