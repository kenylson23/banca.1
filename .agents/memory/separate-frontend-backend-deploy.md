...
**Why:** Relative API calls and default `SameSite=Lax` cookies work locally but fail when Vercel and Railway use different domains; Replit's internal package proxy is not resolvable in a Railway build.

**How to apply:** Keep the frontend URL in a public `VITE_API_URL` build variable, set the frontend origin in Railway's `CORS_ORIGINS`, use `npm ci --registry=https://registry.npmjs.org/` for Railway, and verify `/api/health`, login, session persistence, and `/ws` after both services are deployed.