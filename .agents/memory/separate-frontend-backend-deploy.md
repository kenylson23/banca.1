...
When the frontend and API use separate public domains, they must share one configurable API base URL for every API request and WebSocket connection. Cross-origin session cookies require an explicit CORS allowlist plus `SameSite=None; Secure` in production. A missing or retired Railway hostname makes public data such as plans fail even while the Vercel page itself loads. Lockfiles generated inside Replit may also need public npm tarball URLs before Railway can install them.
...
**How to apply:** For a Railway-only deployment, leave `VITE_API_URL` unset so relative URLs stay same-origin and enable static serving. For a separate Vercel frontend, set `VITE_API_URL` to the currently active Railway service domain and `CORS_ORIGINS` to the corresponding public URLs; verify the API URL returns JSON from `/api/subscription-plans`, not a Railway edge `Application not found` page. Use `npm install --registry=https://registry.npmjs.org/` for Railway and verify `/api/health`, login, session persistence, and `/ws`.