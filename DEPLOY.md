...
**Start command:** `SERVE_STATIC=false NODE_ENV=production node dist/index.js`
...
|---|---|
| `NODE_ENV` | `production` |
| `SERVE_STATIC` | `false` para o Railway funcionar como API/WebSocket separado da Vercel |
...
5. O Start Command já define `NODE_ENV=production` e `SERVE_STATIC=false` para manter o Railway como API/WebSocket. O Railway fornece `PORT` automaticamente.
